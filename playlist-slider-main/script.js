...
function closeModal() {
  ...
  // Start playing from currentIndex after modal is closed
...
const songs = [
  {
    title: "Love Me Not",
    artist: "Ravyn Lenae (Feat. Rex Orange County",
    src: "audio/love-me-not.mp3"
  },
  {
    title: "Meet You Again",
    artist: "Ebony Loren",
    src: "audio/meet-you-again.mp3"
  },
  {
    title: "ชอบทรงไหน",
    artist: "BigBabyBoy",
...
const durations = [188, 160, 151, 164, 196];
const startTimes = [100, 37, 71, 34, 11];
...
function updateActiveSong(index) {
  items.forEach((item, i) => {
    if (i === index) {
      ...
      // Load audio and start from startTime
...
document.querySelectorAll(".progress-bar").forEach((bar, i) => {
  bar.addEventListener("click", (e) => {
    ...
    const totalTime = durations[i] - startTimes[i];
    const newTime = startTimes[i] + (clickPct * totalTime);

    if (i === currentIndex) {
      audioPlayer.currentTime = newTime;
      const newProgress = ((newTime - startTimes[i]) / totalTime) * 100;
      startProgressBar(progressEls[i], i, newProgress);
    } else {
      currentIndex = i;
      updateActiveSong(currentIndex);
    }
...
function updateModalTheme() {
  ...
  tempImg.onload = () => {
    ...
    modalContent.style.background = `linear-gradient(to bottom right, rgb(${r}, ${g}, ${b}), #ffffff)`;
    modalContent.style.borderColor = `rgb(${Math.max(r - 40, 0)}, ${Math.max(g - 40, 0)}, ${Math.max(b - 40, 0)})`;
    modalContent.style.color = `rgb(${Math.max(r - 40, 0)}, ${Math.max(g - 40, 0)}, ${Math.max(b - 40, 0)})`;
  };
}
...
// Make sure clicking a song card (radio) updates the actual song
items.forEach((item, i) => {
  item.addEventListener("change", () => {
    if (item.checked && i !== currentIndex) {
      currentIndex = i;
      updateActiveSong(i);
    }
  });
